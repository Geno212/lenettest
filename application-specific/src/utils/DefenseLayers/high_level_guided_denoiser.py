...
class HGD(Module):
    def __init__(
        ...
        forward_path_info = {
            ...
            "layers_inputs": list(
                map(lambda x: int(x * width), [64, 64, 128, 256, 256]),
            ),
            "layers_outputs": list(
                map(lambda x: int(x * width), [64, 128, 256, 256, 256]),
            ),
        }
        backward_path_info = {
            "num_layers": [3, 3, 3, 2],
        ...
        for i, (num_layers, inp, out) in enumerate(
            zip(
                forward_path_info["num_layers"],
                forward_path_info["layers_inputs"],
                forward_path_info["layers_outputs"], strict=False,
            ),
        ):
        ...
        for i, (num_layers, inp, out) in enumerate(
            zip(
                backward_path_info["num_layers"],
                backward_path_info["layers_inputs"],
                backward_path_info["layers_outputs"], strict=False,
            ),
        ):
